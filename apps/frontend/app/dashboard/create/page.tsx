...
export default function CreateAssignmentPage() {
  ...
  const addQuestionRow = () => setQuestions([...questions, { id: Date.now().toString(), type: "", count: 1, marks: 1 }]);
  const removeQuestionRow = (id: string) => setQuestions(questions.filter((q) => q.id !== id));
  ...
  useEffect(() => {
    ...
    // 1. Initial check (in case backend finished before socket joined)
    checkAssignmentStatus();

    // 2. Start a safety poll every 3 seconds to guarantee we never get stuck
    const pollInterval = setInterval(() => {
      if (step === 2) checkAssignmentStatus();
    }, 3000);

    // 3. Connect to WebSocket for instant updates
    if (socket) {
      socket.emit("join-assignment", createdAssignmentId);

      const handleAiUpdate = (data: { assignmentId: string; status: any }) => {
        if (data.assignmentId !== createdAssignmentId) return;
        
        if (data.status === "COMPLETED") {
          toast.success("AI Generation Complete!");
          addNotification({ type: "success", title: "Assignment ready", message: "The assignment finished generating." });
          checkAssignmentStatus(); // Fetch the final JSON
        } else {
  ...
  }, [socket, createdAssignmentId, step, checkAssignmentStatus, addNotification]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const formattedConfig = questions.map((q) => ({ questionType: q.type, count: q.count, marks: q.marks }));
      ...
      setStep(2); // Switch to Generating UI
  ...
  const handleDownloadPdf = async () => {
    // If we already know the URL is ready, just open it
    if (pdfUrl && pdfStatus === "COMPLETED") {
      window.open(pdfUrl, "_blank");
      toast.success("Downloading PDF...");
      addNotification({ type: "success", title: "PDF Download", message: "Opening your PDF." });
      return;
    }
    
    try {
      setPdfStatus("PROCESSING"); // Optimistically set UI to loading
      const response = await assignmentService.downloadPdf(createdAssignmentId!);
      
      // FIX: If the backend says it's already done, open it instantly!
      if (response.pdfUrl) {
        ...
        // If it actually went to the background queue
        ...
        checkAssignmentStatus(); // Trigger a DB sync just in case
      }
    } catch {
      setPdfStatus("FAILED");
  ...
  return (
    <div className="mx-auto max-w-4xl pb-10">
      ...
      {/* Header & Progress Bar (Visible ONLY in Step 1 & 2) */}
      {step !== 3 && (
        ...
        <div className="bg-white rounded-[28px] p-5 md:p-8 shadow-[0_18px_60px_-36px_rgba(0,0,0,0.35)] border border-white/80">
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors mb-6" onClick={() => fileInputRef.current?.click()}>
                ...
                {questions.map((q) => (
                  <div key={q.id} className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_120px_120px_28px] items-center gap-3">
                    <div className="flex-1">
                      <select value={q.type} onChange={(e) => updateQuestion(q.id, "type", e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none appearance-none">
                    ...
                    {questions.length > 1 && (
                      <button onClick={() => removeQuestionRow(q.id)} className="flex h-7 w-7 items-center justify-center rounded-full text-gray-400 hover:bg-red-50 hover:text-red-500 transition-colors md:justify-self-end">
                        <X className="w-5 h-5" />
                      </button>
                    )}
                  </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                <button onClick={() => router.back()} className="flex items-center gap-2 px-6 py-3 border border-gray-200 rounded-full text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors">
                  <ArrowLeft className="w-4 h-4" /> Previous
                </button>
                <button onClick={handleSubmit} disabled={isSubmitting} className="flex items-center gap-2 px-8 py-3 bg-[#1c1c1e] text-white rounded-full text-sm font-bold hover:bg-black transition-colors disabled:opacity-70 shadow-lg">
            ...
            <div className="animate-in fade-in zoom-in-95 duration-500 min-h-[400px]">
              ...
              {generationStatus === "FAILED" && (
                <div className="flex flex-col items-center justify-center h-[400px] text-center">
                  ...
                  <button onClick={() => setStep(1)} className="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-900 font-bold rounded-full transition-colors text-sm">
                    Go Back & Try Again
                  </button>
                </div>
              )}
      ...
      {/* ======================= STEP 3: COMPLETED (Dark Banner + Paper) ======================= */}
      {step === 3 && generatedPaper && (
        <div className="animate-in fade-in zoom-in-95 duration-500 space-y-6">
          ...
          <div className="border border-gray-200 rounded-[24px] p-8 md:p-14 bg-white shadow-sm font-sans text-gray-900">
            ...
            <div className="font-bold mb-8 text-[15px]">
                ...
                <ol className="list-decimal pl-5 space-y-4 text-[14px]">
                  {section.questions.map((q: any, qIdx: number) => (
                    <li key={qIdx} className="pl-2 leading-relaxed">
                      {q.difficulty ? `[${q.difficulty}] ` : ""}{q.text} <span className="font-normal">[{q.marks} Marks]</span>
                    </li>
                  ))}
                </ol>
              </div>
            ...
            <div className="pt-4">
              ...
              <ol className="list-decimal pl-5 space-y-5 text-[14px]">
                {generatedPaper.sections?.flatMap((s: any) => s.questions).map((q: any, idx: number) => (
...
// Helper block preserved from ViewAssignmentPage
